...
const ClassCard = ({ course_title, faculty_name, attendance_percent, mt_marks, et_marks, total_held, code, attended_classes }) => {
  ...
  return (
    <div className="p-4 rounded-xl shadow-md bg-white border border-gray-200 hover:shadow-lg transition duration-300">
      ...
      <div className="grid grid-cols-2 gap-3 text-sm font-semibold">
        ...
        {/* Status Indicator */}
        <div className="bg-blue-50 text-blue-700 border border-blue-200 p-3 rounded-lg text-center">
          Status: <span className="font-extrabold text-lg">In Progress</span>
        </div>
      </div>
...
const CurrentClasses = () => {
  ...
  const fetchClasses = async () => {
    ...
    try {
      ...
      // Filter: Show classes where a final grade is NOT assigned
      const current = allClasses.filter(cls => !cls.is_completed);
      setClasses(current);
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="p-6 text-center text-gray-500 min-h-full">
        <svg className="animate-spin h-5 w-5 mr-3 inline-block text-blue-600" viewBox="0 0 24 24">...</svg>
  ...
  return (
    <div className="p-6 bg-white rounded-2xl shadow-lg min-h-full">
      <h2 className="text-2xl font-bold text-gray-800 mb-8 flex items-center border-b pb-3">
        <span className="text-3xl mr-3">📘</span> Current Semester Classes
      </h2>
